...
vars url_postfix = '.html';
vars url_map;
unless url_map.isproperty then
	newmapping([],20,false,true) -> url_map;
endunless;
vars type_postfix;
unless type_postfix.isproperty then
	newmapping([],20,url_postfix,true) -> type_postfix;
endunless;
'' -> type_postfix('lib');
vars last_url = false;
vars body_preamble, body_postamble, mansearch_url;
if body_preamble.isundef then false->body_preamble; endif;
if body_postamble.isundef then false->body_postamble; endif;
if mansearch_url.isundef then 'http://duckduckgo.com/?q=linux+man+' -> mansearch_url; endif;
...
define findhelpfile(system,type,name )-> file;
lvars list,fext;
    if system = "pop11" then
        ...
        case 'lib'   then popuseslist -> list;
            ...
            warning('Invalid Type "'><type><'" expected help, ref, teach or doc',[^type]);
			false -> file;
            return();
        endswitchon;
    else
...
define get_longest_match(path,fileext,hash);
lvars res,filepos,file='';
    unless path.isstring then return(false); endunless;
    sysfileok(path) -> path;
    if hash(path) ->> res then
		if fileext then
			return(res><fileext);
		else
			return(res);
    ...
    sys_fname(path)(4) -> filepos;
    if filepos = 1 then return(false) endif;
    substring(filepos,length(path)-filepos+1,path) -> file;
    if fileext then
		file >< fileext -> file;
	endif;
    substring(1,filepos-1,path) -> path;
    repeat
       if hash(path) ->> res then
           return(res dir_>< file);
       endif;
       sys_fname(path)(4) -> filepos;
       quitif(filepos <2);
       substring(filepos,length(path)-filepos+1,path) dir_>< file -> file;
       substring(1,filepos-2,path) -> path;
    endrepeat;
    return(false);
enddefine;

define insert_html_links(system,line);
    lvars i j k l line type name system patern file url prev_type = false;
    lconstant types = ['help' 'ref' 'teach' 'doc' 'man' 'unix' 'ploghelp' 'lib'],
        ...
        while (locchar(`*`,k,line) ->> i) do
            ...
            then
                ...
                if type = 'man' or type = 'unix' then
                   mansearch_url><name -> url;
				else
                	;;;find the file for this link
                	if findhelpfile(system,type,name) ->> file then
                    	get_longest_match(file,type_postfix(type),url_map) -> url;
                	else
                    	false -> url;
                	endif;
				endif;
...
define insert_index_links(system,type,line);
    lvars i j k line type system name file url;
    ...
    [%
        while (next_word(k,line,'_.') -> i -> j ->> name) do
			...
			;;;find the file for this link
            if findhelpfile(system,type,name) ->> file then
                get_longest_match(file,url_postfix,url_map) -> url;
            else
				false -> url;
            endif;
            if url then
...
define indextohtml(system,type,name);
    ...
    lconstant end_anchor = consword('</A>');
   	...
   	findhelpfile(system,type,name) -> file;
...
define index_to_html(title,system,type,name,outfile);
    lvars type name file rep list line c s offset = 1, url;
    dlocal poplinewidth = false; ;;;stop line breaks
    lvars new_index = false, contents_list = [], contents = false;
    lconstant end_anchor = consword('</A>');
    dlocal cucharout = discout(outfile);
    ...
    if file then
        ...
        npr('<P>\nFile "'><file><'" not found is system '><system);
		npr('<P>\nUsing list '><list);
    endif;
    pr('</BODY>\n</HTML>\n');
    cucharout(termin);
   ...
   findhelpfile(system,type,name) -> file;
    ...
    if file then
        ...
        for line from_repeater rep do
            if last_url then last_url else '' endif -> url;
...
define print_lines(text);
lvars line, i;
    if text then
    	if text.isstring or text.isword then
			npr(text);
		else
			for line in text do
				if line.isstring then
					...
					for i in line do pr(i); endfor; nl(1);
				endif;
			endfor;
		endif;
	endif
...
define file_to_html(title,system,file,outfile);
    lvars type name file rep list line i index new_index = false, url fext;
    dlocal poplinewidth = false; ;;;stop line breaks
    lvars contents_list = [], contents = false;
    lconstant end_anchor = consword('</A>');
    dlocal pop_file_versions = false;
    dlocal cucharout = discout(outfile);
	...
	print_lines(body_postamble);

    pr('</BODY>\n</HTML>\n');
    cucharout(termin);
enddefine;

define dir_to_html(title,system,indir,outdir);
lvars file,name,rep;
    sys_file_match(sysfileok(indir) dir_>< '*','',false,0) -> rep;
	for file from_repeater rep do
		unless file.sysisdirectory then
            nextunless(readable(file));
			sys_fname_name(file) -> name;
			unless isendstring('index',name) then
				file_to_html(title >< ' ' >< name,system,file,outdir dir_>< name >< url_postfix);
			endunless;
		endunless;
	endfor;
enddefine;
